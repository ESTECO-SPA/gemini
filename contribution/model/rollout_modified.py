...
from external.IRL.Auxiliary_functions import Variational_Generator
from gemini.dataset import PlotFiskhamnsmotet
from gemini.resources import get_path_data_file


# This  is the rollout os Santiago with the new nearest obstacles algorithm (as implemented in Gemini).

def policy_rollout(b_states, flat_o_states, G, n_agents, visitor: dict):
    ...
    with torch.no_grad():
        for i in range(len(b_states) - 1):
            # [REF2] nearest_obstacles are identified on the fly.
            nearest_obstacles = find_nearest_obstacles(l_a_state[i].reshape(1, -1), flat_o_states[i], n_agents)
            current_state = torch.cat([torch.Tensor(nearest_obstacles.reshape(1, -1)),
...
def par_roll(test_ids, sel_routes, obstacles, G, visitor):
    ...
    for i in test_ids:
        ...
        if (len(ac) > 3) & (len(ac) < 400):
            ac_states, _, flat_o_states = get_individual_states(ac, obstacles)
            roll_outs.append(policy_rollout(ac_states, flat_o_states, G, 2, visitor))
            te.append(i)
...
def get_individual_states(b, obstacles):
    ...
    state_space = state_space[state_space['member'] != b.iloc[0].member]
    n_agents = len(state_space['member'].unique())
    ...
    return b_states, b_actions, flatten(obstacle_states, n_agents)
...
with open(get_path_data_file("rollout_modified.pickle"), "wb") as file:
